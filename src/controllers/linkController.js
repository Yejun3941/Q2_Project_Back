...
exports.getAllLinks = async (req, res) => {
  try {
    ...
    course ? (whereCondition.course = fermatDecode(course)) : null; // 코스 아이디 있을 경우 조회 조건에 추가
    spot ? (whereCondition.spot = fermatDecode(spot)) : null; // 스팟 아이디 있을 경우 조회 조건에 추가
...
exports.updateLink = async (req, res) => {
  ...
  try {
    ...
    link.course = course ? fermatDecode(course) : link.course;
    link.spot = spot ? fermatDecode(spot) : link.spot;
