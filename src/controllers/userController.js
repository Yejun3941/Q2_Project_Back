...
exports.getUserById = async (req, res) => {
  ...
  try {
    const user = await User.findByPk(id); // 주어진 ID로 유저 조회
    if (!user) {
      return res.status(404).json({ error: "User not found" }); // 유저가 없을 경우 404 상태 코드 반환
    }
    res.json(user); // 조회된 유저를 JSON 형태로 응답
  } catch (err) {
...
exports.updateUser = async (req, res) => {
  const { id } = req.params; // URL 파라미터에서 유저 ID 추출
  const { email, nickname, password, provider } = req.body; // 요청 바디에서 업데이트할 데이터 추출
  try {
    const user = await User.findByPk(id); // 주어진 ID로 유저 조회
    if (!user) {
      return res.status(404).json({ error: "User not found" }); // 유저가 없을 경우 404 상태 코드 반환
    ...
    res.json(user); // 업데이트된 유저를 JSON 형태로 응답
  } catch (err) {
...
exports.deleteUser = async (req, res) => {
  ...
  try {
    ...
    res.json({ message: "User deleted successfully (soft delete)" }); // 삭제 완료 메시지 반환
  } catch (err) {
